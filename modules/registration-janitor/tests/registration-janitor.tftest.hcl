...
run "safe_defaults_and_delayed_confirmation" {
  ...
  command = apply
  assert {
    condition     = jsondecode(aws_iam_role_policy.cleanup.policy).Statement[1].Action == ["ssm:GetParameters"]
  ...
  assert {
    condition     = jsondecode(aws_iam_role_policy.cleanup.policy).Statement[0].Action == ["ec2:DescribeInstances"]
    error_message = "The janitor must not be able to terminate instances."
  }
  assert {
    condition     = toset(jsondecode(aws_iam_role_policy.cleanup.policy).Statement[0].Condition.StringEquals["aws:RequestedRegion"]) == toset(["eu-west-1", "us-east-1"])
    error_message = "EC2 access must be limited to the configured regions."
  }
  assert {
    condition     = length(jsondecode(aws_iam_role_policy.cleanup.policy).Statement) == 3
...
run "customer_managed_key" {
  ...
  assert {
    condition     = jsondecode(aws_iam_role_policy.cleanup.policy).Statement[3].Action == ["kms:Decrypt"] && jsondecode(aws_iam_role_policy.cleanup.policy).Statement[3].Resource == var.config.github_app_kms_key_arn
...
run "additional_apps_manifest" {
  ...
  assert {
    condition     = toset(jsondecode(aws_iam_role_policy.cleanup.policy).Statement[1].Action) == toset(["ssm:GetParameter", "ssm:GetParameters"]) && toset(jsondecode(aws_iam_role_policy.cleanup.policy).Statement[1].Resource) == toset([var.config.github_app_parameters.id.arn, var.config.github_app_parameters.key_base64.arn, var.config.github_app_parameters.additional_apps_manifest.arn, "arn:aws:ssm:eu-west-1:123456789012:parameter/app/extra/key"])
